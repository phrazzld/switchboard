...
fn test_unix_directory_permissions_for_new_directories() {
    ...
    let log_path = temp_dir.join("permissions.log").to_string_lossy().to_string();
    ...
    assert!(parent_dir.exists() && parent_dir.is_dir(), "Directory should be created");
...
fn test_unix_permissions_on_existing_directory() {
    ...
    let log_path = temp_dir.join("app/permissions.log").to_string_lossy().to_string();
    let config = create_test_config(&log_path, LogDirectoryMode::Default);
    ...
    assert!(parent_dir.exists() && parent_dir.is_dir(), "Directory should still exist");
...
fn test_directory_creation_for_different_environments() {
    ...
    // Resolve paths
    let default_resolved = default_resolver.resolve().expect("Failed to resolve default path");
    let xdg_resolved = xdg_resolver.resolve().expect("Failed to resolve XDG path");
    let system_resolved = system_resolver.resolve().expect("Failed to resolve system path");
...
fn test_recreate_directory_with_existing_file() {
    ...
    fs::write(&conflict_dir, "This is a file, not a directory")
        .expect("Failed to create file");
    
    // Now create a config that will need this to be a directory
    let log_path = temp_dir.join("logs/app/nested/conflict.log").to_string_lossy().to_string();
    let config = create_test_config(&log_path, LogDirectoryMode::Default);
