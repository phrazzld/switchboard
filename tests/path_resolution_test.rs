...
use switchboard::{
    ...
    logger::{
        get_environment_log_directory, get_xdg_log_directory, APP_LOG_SUBDIR, DEFAULT_LOG_DIR,
        LogEnvironment, LogPathResolver, LogType, SYSTEM_LOG_DIR, TEST_LOG_SUBDIR,
    },
};
...
fn create_test_config(
    log_file_path: &str,
    log_directory_mode: LogDirectoryMode,
) -> Config {
    Config {
        port: "0".to_string(),
...
impl TestLogResolver {
    ...
    fn resolve_and_verify(&self) -> PathBuf {
        ...
        let filename = self.original_config.log_file_path.split('/').last().unwrap_or(&self.original_config.log_file_path);
        assert!(
...
fn test_resolve_with_legacy_path() {
    ...
    assert!(
        resolved_path.parent().unwrap().to_string_lossy().contains(APP_LOG_SUBDIR),
...
fn test_subdir_creation() {
    ...
    let config = Config {
        ...
        log_format: "pretty".to_string(),
        log_bodies: true,
        log_file_path: temp_dir.join("subdir-test.log").to_string_lossy().to_string(),
        log_file_level: "debug".to_string(),
        log_max_body_size: 1024,
    ...
    assert!(
        ...
        "App subdirectory should have been created in {}", log_dir.display()
    );
...
fn test_different_environments() {
    ...
    assert_eq!(dev_dir, PathBuf::from(DEFAULT_LOG_DIR), "Development environment should use DEFAULT_LOG_DIR");
    assert_eq!(user_dir, get_xdg_log_directory(), "UserInstallation environment should use XDG directory");
    assert_eq!(system_dir, PathBuf::from(SYSTEM_LOG_DIR), "SystemService environment should use SYSTEM_LOG_DIR");
...
fn test_unix_directory_permissions() {
    ...
    let config = Config {
        ...
        log_format: "pretty".to_string(),
        log_bodies: true,
        log_file_path: temp_dir.join("permissions-test.log").to_string_lossy().to_string(),
        log_file_level: "debug".to_string(),
        log_max_body_size: 1024,
        log_directory_mode: LogDirectoryMode::Default,
    };
