...
fn test_app_test_log_separation() {
    ...
    assert!(verify_directory_structure(), "Directory structure is incorrect");
    ...
    let test_canonical = fs::canonicalize(&test_log_path).expect("Failed to canonicalize test path");
